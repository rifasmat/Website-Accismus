...
<div class="container">
    ...
    <div>
        <a href="{{ route('senate.history-rf.list') }}" class="btn btn-secondary btn-sm">Batal</a>
        <form method="POST" action="{{ route('senate.history-rf.destroy', $history->history_uuid) }}" style="display:inline;">
            @csrf
            @method('DELETE')
